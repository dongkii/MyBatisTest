...
public class MemberDao {
	...
	public Member selectMember(Member m){
		...
		try {
			/* 마이바티스 config 설정파일의 내용을 읽어서 db 연결
			...
			member = session.selectOne("member.selectMember1", m);			
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
			session.close();
	...
	public Member selectMember(String userId) {
		...
		try {
			session = new SqlSessionFactoryBuilder().build(Resources.getResourceAsStream("mybatis/mybatis-config.xml")).openSession(false);
			member = session.selectOne("member.selectMember2",userId);
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
			session.close();
	...
	public ArrayList<Member> selectList() {
		...
		try {
			session = new SqlSessionFactoryBuilder().build(Resources.getResourceAsStream("mybatis/mybatis-config.xml")).openSession(false);
			list = session.selectList("member.selectList");
		} catch (Exception e) {
			e.printStackTrace();
		}finally{
			session.close();
